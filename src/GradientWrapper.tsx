...
const GradientWrapper: React.FC<Props> = ({
  ...
  const buildRgbString = (r: number, g: number, b: number) => `rgb(${Math.round(r)},${Math.round(g)},${Math.round(b)})`;
  ...
  const parseColorToRgb = (color: string): { r: number; g: number; b: number } => {
    ...
    if (color.startsWith('#')) {
      ...
      if (cleaned.length !== 6) throw new Error(`Invalid hex color: ${color}`);
      return {
    ...
    if (rgbMatch) {
      ...
      if ([r, g, b].some(n => isNaN(n))) throw new Error(`Invalid RGB values: ${color}`);
      return { r, g, b };
    }
  ...
  useEffect(() => {
    if (!animated || !backgroundGradient || backgroundGradient.length < 2) return;
  
    const speed = backgroundRotationSpeed || 500;
  
    const extendedColors = [...backgroundGradient, backgroundGradient[0]];
    const shiftedColors = [...backgroundGradient.slice(1), backgroundGradient[0], backgroundGradient[1]];
    ...
    const listenerId = animatedColorValue.addListener(({ value }) => {
      ...
      const r2 = rgbList[nextIndex].r + (rgbList[(nextIndex + 1) % rgbList.length].r - rgbList[nextIndex].r) * fraction;
      const g2 = rgbList[nextIndex].g + (rgbList[(nextIndex + 1) % rgbList.length].g - rgbList[nextIndex].g) * fraction;
      const b2 = rgbList[nextIndex].b + (rgbList[(nextIndex + 1) % rgbList.length].b - rgbList[nextIndex].b) * fraction;

      setAnimatedGradient([
        buildRgbString(r, g, b),
        buildRgbString(r2, g2, b2),
      ]);
    });
  ...
  const appliedBackground = backgroundGradient ?? (isDark ? defaultDarkBackground : defaultLightBackground);
  ...
  const fullBorderGradient = typeof fullBorderGradientResult === 'object' ? fullBorderGradientResult.colors : fullBorderGradientResult;
  const calculatedBorderLocations = typeof fullBorderGradientResult === 'object' ? fullBorderGradientResult.locations : borderLocations;
  ...
  const { start: bgStart, end: bgEnd } = angle !== undefined
